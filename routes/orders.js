...
router.post('/createorder',isLoggedIn,async (req,res)=>{
    ...
    try {
        ...
        if(req.query.is_coupon_applied == 'y') {
            ...
            const checkDiscountCouponBalance = await request.query('Select discount_coupon from PrimeUsersWallet where prime_user_id=@user_id');
            if (checkDiscountCouponBalance.recordset[0].discount_coupon < calculations.totalDiscountPrice) 
        ...
        request.input('coupon',sql.Bit,req.query.is_coupon_applied=='y'?1:0);
        ...
        if (req.query?.order_type=='cod') {
            ...
            const result = await request.query(`DECLARE @OrderIdTable TABLE(order_id nvarchar(50)); 
            Insert into 
            Orders(ordered_month, 
                ...
                OUTPUT Inserted.order_id into @OrderIdTable 
                values(@month,
                    @year,
                    @user_id,
                    @total_amount,
                    ...
                    Select order_id from @OrderIdTable`);
            await SendOrderConfirmationMSG({order_id : result.recordset[0].order_id, amount : parseFloat(calculations.overallTotal), user : req.user});
            ...
            if(updateWallet.rowsAffected[0]==1) {
                ...
                const phoneQuery = `Select ${req.user.type==='prime'?'user_mobile_number as num from PrimeUsers':'phone as num from users'} where user_id=@user_id`
                const result = await request.query('DECLARE @OrderIdTable TABLE(order_id nvarchar(50)); Insert into Orders(ordered_month, ordered_year,user_id,total_amount,payment_status,payment_mode,order_status,addr_id,is_coupon_applied,delivery_charge,total_discount,user_type) OUTPUT Inserted.order_id into @OrderIdTable values(@month,@year,@user_id,@total_amount,@pay_status,@pay_mode,@order_status,@addr_id,@coupon,@delivery_charge,@discount,@type);Select order_id from @OrderIdTable');
